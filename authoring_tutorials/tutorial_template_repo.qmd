...
For Mac users, any `.DS_Store` files are also automatically ignored--those should never be tracked in a git project anyway as they are local indexing files for searching the system. (Tip: run the command `usethis::git_vaccinate()` if you want to globally ignore a handful of annoying files)
...
* `version` should begin with 0.0.1 following typical semantic versioning rules. Do not update this until after making the initial Zenodo connection (discussed in a later chapter)
...
* `url` should be the address for the actual website i.e. the GitHub Pages


### LICENSE.md and LICENSE-CODE.md {#licenses}

Our recommended licenses are CC-BY-SA 4.0 for the written text and CC-0 for code contained within the tutorials. These license files are contained in `LICENSE.md` and `LICENSE-CODE.md` respectively.

### README.md {#readme}

This README file explains what this project is about in simple terms. It gives a short overview of the project and easy-to-follow instructions for running code and installing any additional software if needed.
...
This file might seem complex, but you only need to focus on a few sections. I'll explain it using four main parts: `project`, `website`, `format`, and `editor`.
...
This section specifies the navbar (navigation bar) at the top of the window. Only update the URLs to include the correct GitHub repository address.
...
This section specifies additional website formatting and styling decisions and requires no change. (However, if you think there are broad styling changes or additions we should make for all of our tutorials, feel free to open an issue or a PR!)
